...
@section('title', 'Edit Student - CollegeOS')
...
@section('content')
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            ...
            <p class="topbar-sub">Update student information</p>
        </div>
    ...
    <div class="card">
        <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid">
                <div>
                    <div class="form-group">
                        ...
                        <input type="text" name="student_name" class="form-input" value="{{ old('student_name', $student->student_name) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="date_of_birth" class="form-input" value="{{ old('date_of_birth', optional($student->date_of_birth)->format('Y-m-d')) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="father_name" class="form-input" value="{{ old('father_name', $student->father_name) }}" required>
                        @error('father_name') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Contact Number (10 digits) *</label>
                        <input type="tel" name="contact_number" class="form-input" value="{{ old('contact_number', $student->contact_number) }}" required>
                        @error('contact_number') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Alternate Contact</label>
                        <input type="tel" name="alternate_contact_number" class="form-input" value="{{ old('alternate_contact_number', $student->alternate_contact_number) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="aadhaar_number" class="form-input" maxlength="12" value="{{ old('aadhaar_number', $student->aadhaar_number) }}" required>
                        @error('aadhaar_number') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Last Class Studied *</label>
                        <input type="text" name="last_class_studied" class="form-input" value="{{ old('last_class_studied', $student->last_class_studied) }}" required>
                        @error('last_class_studied') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Previous Year Grade *</label>
                        <input type="text" name="previous_year_grade" class="form-input" value="{{ old('previous_year_grade', $student->previous_year_grade) }}" required>
                        @error('previous_year_grade') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Last School Studied *</label>
                        <input type="text" name="last_school_studied" class="form-input" value="{{ old('last_school_studied', $student->last_school_studied) }}" required>
                ...
                <div>
                    <div class="form-group">
                        ...
                        <div class="photo-upload">
                            <img id="photoPreview" src="{{ $student->photo ? asset('uploads/students/' . $student->photo) : 'https://via.placeholder.com/120x150?text=Passport+Photo' }}" class="photo-preview">
                            <input type="file" name="photo" accept="image/jpeg,image/jpg,image/png" id="photoInput" onchange="previewPhoto(event)">
            ...
            <div style="display: flex; gap: 12px; margin-top: 24px; justify-content: flex-end;">
                ...
                <button type="submit" class="btn-submit"><i class="ti ti-check"></i> Update Student</button>
            </div>
        </form>
    </div>
